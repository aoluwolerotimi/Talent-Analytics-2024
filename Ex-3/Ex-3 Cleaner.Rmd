...
In some other analytical exploration (included at the end of the notebook), attempting to do some fixed effects regression, discovered both N/As and duplicate examiner-id and quarter pairs in the applications dataset. So we begin here with addressing that.

```{r}
# applications %>%
#   summarise_all(~ sum(is.na(.)))
# First addressing N/As
applications <- applications[!is.na(applications$examiner_id) & !is.na(applications$quarter), ]

# Then addressing duplicates, ensuring we don't lost record of a turnover
cleaned_applications <- applications %>%
  select(-au_move) %>% # Drop the au_move column, focus of the predictive model will be separation indicator
...
# applications %>%
#   summarise_all(~ sum(is.na(.)))
# 9229 N/As in examiner_id. will need to go back and investigate, but for now focusing on learning objective of predictive modeling and evaluating performance
