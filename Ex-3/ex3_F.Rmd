...
columns_to_exclude <- c(
  ...
  "Hispanic_in_art_unit", "Other_in_art_unit", "White_in_art_unit"
)
...
quarter_df <- df %>%
  ...
  select(examiner_id, quarter, latest_date, separation_indicator, PEN_applications, ISSUED_applications, PEN_applications, abn_applications, gender, race, tenure_days, au_moves) %>%
...
-   The model exhibits a high true positive rate over most specificity levels, which means it is adept at correctly identifying the positive class ("X1").
...
Based on these results, you might consider the following recommendations:

1.  **Focus on Specificity**: Since the specificity is higher than sensitivity, the model is better at predicting non-exits (X1). If preventing false alarms (predicting an exit when there isn't one) is more important, this model is quite suitable. However, if missing potential exits (predicting no exit when there is one) is more critical, you might want to improve sensitivity.

2.  **Improving Sensitivity**: If you want to capture more true positives (X0), consider revisiting the model's threshold for classification. Lowering the threshold for predicting exits could help capture more true exits at the cost of increasing false positives.

3.  **Feature Importance**: Examine the feature importance scores from the GBM model to identify which factors are most influential in predicting exits. Target interventions on these areas. For instance, if workload or tenure is a significant factor, consider measures to manage workload effectively or provide clearer career progression paths.
